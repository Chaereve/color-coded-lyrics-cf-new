...
test('cả năm mục của trang quản trị đều dựng được (không sót prop → trang trắng)', async () => {
  for (const tab of ['pending', 'active', 'expired', 'orders', 'done', 'media', 'comments']) {
...
test('dải số liệu đếm ĐÚNG thứ mà từng mục sẽ liệt kê — và đếm một lần', async () => {
  ...
  assert.deepEqual(nums, ['2', '1', '0', '1', '2', '1', '0'],
    'chờ duyệt 2 · đang xử lý 1 · hết hạn 0 · đơn chờ 1 · đã xong/từ chối 2 · video 1 · bình luận 0')
