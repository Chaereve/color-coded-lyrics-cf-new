...
console.log('KPI:', kpis.map(k => k.querySelector('.k')?.textContent + '=' + k.querySelector('.v')?.textContent).join(' | '))
for (const kpi of kpis) {
  await click(kpi)
  await tick(500)
  const h2 = q('.adm-h2')?.textContent?.replace(/\s+/g, ' ').trim()
  console.log(`\n== tab ${kpi.querySelector('.k')?.textContent} == url=${window.location.search}`)
  console.log('   h2:', h2 || '(KHÔNG CÓ)')
  console.log('   bar:', !!q('.adm-bar'), '| searchwrap:', !!q('.adm-bar .searchwrap'), '| note:', !!q('.adm-note'), '| keys:', !!q('.adm-keys'))
  console.log('   panel:', !!q('.adm-panel'), '| rows .adm:', qa('.adm').length, '| media rows:', qa('[class*=adm-media]').length)
  console.log('   empty:', q('.empty')?.textContent?.replace(/\s+/g, ' ').trim() || '-')
  console.log('   children của .adm-page:', [...(q('.adm-page')?.children || [])].map(c => c.tagName.toLowerCase() + '.' + String(c.className).split(' ')[0]).join(' '))
}
console.log('\nLỖI BẮT ĐƯỢC:', errs.length ? errs.slice(0, 8).join('\n  ') : '(không)')
