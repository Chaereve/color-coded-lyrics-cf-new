...
await click(qa('.adm-kpi')[4])
await tick(500)
console.log('--- media panel head ---')
console.log(q('.adm-panel')?.outerHTML?.replace(/\s+/g, ' ').slice(0, 1500))
console.log('\n--- class names trong panel ---')
const names = new Set()
for (const el of qa('.adm-panel *')) for (const c of String(el.className || '').split(/\s+/)) if (c) names.add(c)
