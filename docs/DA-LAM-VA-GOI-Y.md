...
| Render ném lỗi → **trang đen** (chủ dự án gặp thật ở `:5173`) | React 19 gỡ sạch cây DOM: người dùng chỉ còn nền tối, không biết app hỏng hay mạng hỏng, và cũng không có gì để báo lại | Hai tầng: **lưới render** (`components/ErrorBoundary.jsx` — câu giải thích + nút *Reload* + thông điệp lỗi thật) và **lưới nạp** (`index.html` — `#root` trống 8 giây thì hiện `.bootfail`). Kèm theo: bản xem trước trong sandbox nay phục vụ **`dist/` dựng sẵn** (`vite preview`) chứ không phải vite dev, vì proxy của môi trường chặn WebSocket của `/@vite/client` và trình duyệt treo trước khi app kịp render |
...
| `src/components/ErrorBoundary.test.js` (4 ca) | Màn lỗi dựng được cả khi lỗi không phải `Error`, không lộ `undefined`/`[object Object]`; **và** cả hai lưới an toàn thật sự có mặt trong `main.jsx` / `index.html` (lưới không được bọc thì vô dụng) |
...
| **Lưới an toàn, hai tầng** | `components/ErrorBoundary.jsx` bọc cả app trong `main.jsx`: **render** ném lỗi thì thay vì trang đen là câu giải thích + nút tải lại + thông điệp lỗi thật (gấp trong `<details>`). Tầng thứ hai nằm ngay trong `index.html`: nếu module không nạp nổi (tab cũ sau khi vite đổi tên file, mạng chặn) thì React không bao giờ dựng và `#root` ở lại trống — sau **8 giây** vẫn trống thì hiện `.bootfail` kèm mẹo `Ctrl+Shift+R`. `main.jsx` bật cờ `window.__cclBoot` ngay khi render xong nên lưới này im lặng khi mọi thứ bình thường |
